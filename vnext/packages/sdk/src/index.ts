...
export interface Entry { registration: Registration; handler: Handler; signal: AbortSignal }
...
export class Contributions {
  ...
  add(ctx: Context, kind: string, name: string, handler: Handler, tool: ToolDefinition | null = null) {
    ...
    const entry = { registration: { ...owner, kind, name, tool }, handler, signal: lifetime.signal }
    return ctx.effect(() => {
...
export class Morrow extends Service {
  ...
  model(name: string, execute: (request: PreparedRequest, run: RunContext) => Promise<Message>) { return this.contributions.add(this.ctx, 'model', name, execute) }
...
export class RunContext {
  ...
  async model(header: RequestHeader, options: { purpose?: string; surface?: string[]; temporary?: Message[] } = {}) {
    this.active()
